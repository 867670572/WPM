...
@interface WPMCalendarScrollView () <UICollectionViewDataSource, UICollectionViewDelegate>
...
@implementation WPMCalendarScrollView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if ([super initWithFrame:frame]) {
        ...
        self.contentSize = CGSizeMake(3 * SELF_WIDTH, SELF_HEIGHT);
        [self setContentOffset:CGPointMake(SELF_WIDTH, 0.0) animated:NO];
...
- (NSMutableArray *)monthArray {
    ...
    if (_monthArray == nil) {
        ...
        [_monthArray addObject:[[WPMCalendarMonth alloc] initWithDate:previousMonthDate]];
        [_monthArray addObject:[[WPMCalendarMonth alloc] initWithDate:_currentMonthDate]];
        [_monthArray addObject:[[WPMCalendarMonth alloc] initWithDate:nextMonthDate]];
...
- (void)setupCollectionViews {
    ...
    flowLayout.itemSize = CGSizeMake(SELF_WIDTH / 7.0, SELF_WIDTH / 7.0 * 0.85);
    ...
    [_collectionViewL registerClass:[WPMCalendarCell class] forCellWithReuseIdentifier:kCellIdentifier];
    ...
    [_collectionViewM registerClass:[WPMCalendarCell class] forCellWithReuseIdentifier:kCellIdentifier];
    ...
    [_collectionViewR registerClass:[WPMCalendarCell class] forCellWithReuseIdentifier:kCellIdentifier];
...
- (void)notifyToChangeCalendarHeader {
    ...
    WPMCalendarMonth *currentMonthInfo = self.monthArray[1];
...
- (void)refreshToCurrentMonth {
    ...
    [self.monthArray addObject:[[WPMCalendarMonth alloc] initWithDate:previousMonthDate]];
    [self.monthArray addObject:[[WPMCalendarMonth alloc] initWithDate:_currentMonthDate]];
    [self.monthArray addObject:[[WPMCalendarMonth alloc] initWithDate:nextMonthDate]];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    WPMCalendarCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kCellIdentifier forIndexPath:indexPath];
    
    if (collectionView == _collectionViewL) { // 上一个月展示数据
        
        WPMCalendarMonth *monthInfo = self.monthArray[0];
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            cell.todayLabel.text = [NSString stringWithFormat:@"%d", (int)indexPath.row - (int)firstWeekday + 1];
            ...
            if ((monthInfo.month == [[NSDate date] dateMonth]) && (monthInfo.year == [[NSDate date] dateYear])) {
                if (indexPath.row == [[NSDate date] dateDay] + firstWeekday - 1) {
                    cell.todayCircle.backgroundColor = HEXCOLOR(0x19A79D);
                    cell.todayLabel.textColor = [UIColor whiteColor];
                } else {
        ...
        // 补上前后月的日期，淡色显示
            ...
            cell.todayLabel.text = [NSString stringWithFormat:@"%d", totalDaysOflastMonth - ((int)firstWeekday - (int)indexPath.row) + 1];
            cell.todayLabel.textColor = [UIColor colorWithWhite:0 alpha:0];
            cell.todayCircle.backgroundColor = [UIColor clearColor];
        } else if (indexPath.row >= firstWeekday + totalDays) {
            cell.todayLabel.text = [NSString stringWithFormat:@"%d", (int)indexPath.row - (int)firstWeekday - (int)totalDays + 1];
            cell.todayLabel.textColor = [UIColor colorWithWhite:0 alpha:0];
            cell.todayCircle.backgroundColor = [UIColor clearColor];
        }
        ...
        WPMCalendarMonth *monthInfo = self.monthArray[1];
        ...
        // 补上前后月的日期，淡色显示
            ...
            WPMCalendarMonth *lastMonthInfo = self.monthArray[0];
            NSInteger totalDaysOflastMonth = lastMonthInfo.totalDays;
            cell.todayLabel.text = [NSString stringWithFormat:@"%d", (int)totalDaysOflastMonth - ((int)firstWeekday - (int)indexPath.row) + 1];
            cell.todayLabel.textColor = [UIColor colorWithWhite:0 alpha:0];
            cell.todayCircle.backgroundColor = [UIColor clearColor];
            cell.userInteractionEnabled = NO;
        } else if (indexPath.row >= firstWeekday + totalDays) {
        ...
        WPMCalendarMonth *monthInfo = self.monthArray[2];
        NSInteger firstWeekday = monthInfo.firstWeekday;
        NSInteger totalDays = monthInfo.totalDays;
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            WPMCalendarMonth *lastMonthInfo = self.monthArray[1];
            NSInteger totalDaysOflastMonth = lastMonthInfo.totalDays;
            cell.todayLabel.text = [NSString stringWithFormat:@"%d", (int)totalDaysOflastMonth - ((int)firstWeekday - (int)indexPath.row) + 1];
            cell.todayLabel.textColor = [UIColor colorWithWhite:0 alpha:0];
            cell.todayCircle.backgroundColor = [UIColor clearColor];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.didSelectDayHandler != nil) {
        ...
        WPMCalendarCell *cell = (WPMCalendarCell *)[collectionView cellForItemAtIndexPath:indexPath];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (scrollView.contentOffset.x < SELF_WIDTH) {
        ...
        // 取数组中的值（该数组还是滑动之前的）
        WPMCalendarMonth *currentMothInfo = self.monthArray[0]; // currentMothInfo 11月
        WPMCalendarMonth *nextMonthInfo = self.monthArray[1]; // nextMonthInfo 12月
        WPMCalendarMonth *olderNextMonthInfo = self.monthArray[2]; // olderNextMonthInfo 1月
        
        // 复用 WPMCalendarMonth 对象
        ...
        WPMCalendarMonth *previousMonthInfo = olderNextMonthInfo; // previousMonthInfo == olderNextMonthInfo == 10月份信息
    ...
    // 向左滑动
    else if (scrollView.contentOffset.x > SELF_WIDTH) {
        ...
        WPMCalendarMonth *previousMonthInfo = self.monthArray[1];
        WPMCalendarMonth *currentMothInfo = self.monthArray[2];
        
        
        WPMCalendarMonth *olderPreviousMonthInfo = self.monthArray[0];
        ...
        WPMCalendarMonth *nextMonthInfo = olderPreviousMonthInfo;
    ...
    [scrollView setContentOffset:CGPointMake(SELF_WIDTH, 0.0) animated:NO]; // 然后变换位置
